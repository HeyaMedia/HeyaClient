...
#[cfg(test)]
mod tests {
    use super::super::streaming::SharedBuffer;
    use super::*;
    ...
    fn stream_is_seekable_when_length_is_known_before_probe() {
        ...
        let mut setup =
            probe_stream(StreamingReader::new(buffer), Some("wav")).expect("probe seekable stream");
        let seeked = setup
            .format
            .seek(
                SeekMode::Accurate,
            ...
            .expect("seek stream");
        setup.decoder.reset();
        let decoded = decode_batch(&mut setup, 44_100).expect("decode after seek");

        assert!(seeked.actual_ts > 0);
